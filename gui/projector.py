...
def project(
    ...
    *,
    ...
    z_std = (np.sum((z_samples - z_avg) ** 2) / w_avg_samples) ** 0.5

    # Setup noise inputs.
    noise_bufs = { name: buf for (name, buf) in G.synthesis.named_buffers() if 'noise_const' in name }

    # Load VGG16 feature detector.
    with open('models/vgg16.pkl', 'rb') as f:
    ...
    for step in tqdm(range(num_steps)):
        ...
        synth_images = G(zs, labels, truncation_psi = 1, noise_mode='const')
...
def recreate_aligned_images(src_file,
                            lm, 
                            output_size=1024, 
                            transform_size=4096, 
                            enable_padding=True):
...
def preprocess_image(image_path):
    ...
    face = shape_predictor(img, detector(img)[0])
    parts = face.parts()
    num_landmarks = 68
    landmarks = np.zeros((num_landmarks, 2))
    for k, d in enumerate(parts):
        landmarks[k, :] = [d.x, d.y]
    image  = recreate_aligned_images(image_path, landmarks)
...
def run_projection(
    ...
    target_pil = preprocess_image(target_fname)
